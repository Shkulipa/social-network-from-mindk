    ...
    return (
        <Grid container spacing={3}>
            ...
            <Grid item xs={12} className="block-right">
                {avatarImg ? <img className="avatar-img" src={`http://localhost:3000/images/avatars/${avatarImg}`} alt=""/> : <img className="avatar-img" src={notAvatar} alt=""/>}
            </Grid>
            <Grid item xs={12} className="block-left">
                <Card variant="outlined" >
                    <CardContent>
                        {nameUser && <Typography className="block-left__point" color="textSecondary" gutterBottom>
                            Name : <span className="block-left__point__text">{nameUser}</span>
                        </Typography>}
                        {emailUser && <Typography className="block-left__point" color="textSecondary" gutterBottom>
                            Email : <b><a href={`mailto:${emailUser}`}>{emailUser}</a></b>
                        </Typography>}
